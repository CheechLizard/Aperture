...
async function scanWorkspace() {
  ...
  const unsupportedFiles = files.filter((f2) => f2.parseStatus === "unsupported").length;
  return {
    root,
...
function resolveLuaImport(importPath, fromDir, allFiles, selfPath) {
  ...
  const endMatch = allFiles.find(
    (f2) => (f2.path.endsWith(endPattern) || f2.path === luaPath + ".lua") && f2.path !== selfPath
  );
  ...
  for (let i2 = 0; i2 < parts.length; i2++) {
    const parentPath = parts.slice(0, i2 + 1).join("/");
    for (const ext of luaExtensions) {
...
function analyzeDependencies(files, rootPath) {
  ...
  const codeFiles = files.filter((f2) => isCodeFile2(f2.path));
  debugInfo.push(`Code files: ${codeFiles.length}`);
  const totalRawImports = codeFiles.reduce((sum, f2) => sum + f2.imports.length, 0);
  debugInfo.push(`Raw imports from AST: ${totalRawImports}`);
  const parsed = files.filter((f2) => f2.parseStatus === "parsed").length;
  const unsupported = files.filter((f2) => f2.parseStatus === "unsupported").length;
  const errors = files.filter((f2) => f2.parseStatus === "error").length;
